...
define(["N/record", "N/runtime", "N/file", "N/email", "N/encode", "N/search",
    ...
    function(recordModulo, runtime, fileModulo, email, encode, search, format, log,
        ...
        function GeneracionPDF() {
            ...
            ArrAcumulado = AcumuladoPorConcept();
        ...
        function ObtieneRetencionReteICA() {
            ...
            var idNational_or_Contributory = search.createColumn({
                name: "formulatext",
                formula: "(CASE  WHEN ({custrecord_lmry_br_transaction.custrecord_lmry_ntax} is not null) THEN {custrecord_lmry_br_transaction.custrecord_lmry_ntax}  WHEN ({custrecord_lmry_br_transaction.custrecord_lmry_ccl} is not null) THEN {custrecord_lmry_br_transaction.custrecord_lmry_ccl} ELSE '0'  END)",
                ...
                if (objResult != null) {
                    ...
                    for (var i = 0; i < intLength; i++) {
                        ...
                        if (objResult[i].getValue(columns[13]) != null && objResult[i].getValue(columns[13]) != '- None -' && objResult[i].getValue(columns[13]) != '') {
                            arrAuxiliar[6] = (parseFloat(objResult[i].getValue(columns[13])));
                        } else {
                            arrAuxiliar[6] = 0;
                        }
                        ...
                        if (objResult[i].getValue(columns[14]) != null && objResult[i].getValue(columns[14]) != '- None -' && objResult[i].getValue(columns[14]) != '') {
                            arrAuxiliar[7] = (parseFloat(objResult[i].getValue(columns[14])));
                            if (arrAuxiliar[3]=="VendBill") {
                                arrAuxiliar[7]=Math.abs(arrAuxiliar[7]);
                            }else if(arrAuxiliar[3]=="VendCred"){
                                arrAuxiliar[7]=-Math.abs(arrAuxiliar[7]);
                            }
                        } else {
                            arrAuxiliar[7] = 0;
        ...
        function ObtieneRetencionCabecera(pBusqueda, _cont) {
            ...
            var featuremultib = runtime.isFeatureInEffect({
                feature: "MULTIBOOKMULTICURR"
            });
        ...
        function AcumuladoPorConcept() {
            var arrMoment = new Array();
            var cont = -1;
            for (var i = 0; i <= ArrRetencion.length - 1; i++) {
                var encontro = false;
                //log.debug("AcumuladoPorConcept line",ArrRetencion[i]);
                //Recorrer el arreglo de acumulado x Concepto
                for (var j = 0; j <= arrMoment.length - 1; j++) {
                    //Es el mismo concepto
                    if (arrMoment[j][0] == ArrRetencion[i][0] && arrMoment[j][2] == ArrRetencion[i][3]) {
                        arrMoment[j][1] += parseFloat((ArrRetencion[i][2]).toFixed(2));
                        arrMoment[j][2] = ArrRetencion[i][3];
                        arrMoment[j][3] += parseFloat((ArrRetencion[i][4]).toFixed(2));
                ...
                if (!encontro) {
                    ...
                    arrMoment[cont][0] = ArrRetencion[i][0];
                    arrMoment[cont][1] = parseFloat((ArrRetencion[i][2]).toFixed(2));
                    arrMoment[cont][2] = ArrRetencion[i][3];
                    arrMoment[cont][3] = parseFloat((ArrRetencion[i][4]).toFixed(2));
