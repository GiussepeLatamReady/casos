...
define(["N/record", "N/runtime", "N/file", "N/encode", "N/search",
        "N/format", "N/log", "N/config", "N/task", 'N/render', "N/query", "N/file", "N/xml",
    ...
    function(recordModulo, runtime, fileModulo, encode, search, format, log,
        config, task, render, query, file, xml, libFeature) {
        ...
        function SetPUCMultibook(ArrTemp) {
            ...
            for (var i = 0; i < ArrTemp.length; i++) {
                if (paramDigits == 2) {
                    ...
                    if (jsonAccounts[ArrTemp[i][15]] != null) {
                        ArrTemp[i][0] = jsonAccounts[ArrTemp[i][15]].columna9;
                        ArrTemp[i][1] = jsonAccounts[ArrTemp[i][15]].columna10;
                        ArrTemp[i][2] = jsonAccounts[ArrTemp[i][15]].columna7;
                        ArrTemp[i][3] = jsonAccounts[ArrTemp[i][15]].columna8;
                        ArrTemp[i][4] = jsonAccounts[ArrTemp[i][15]].columna5;
                        ArrTemp[i][5] = jsonAccounts[ArrTemp[i][15]].columna6;
                        ArrTemp[i][6] = jsonAccounts[ArrTemp[i][15]].columna3;
                        ArrTemp[i][7] = jsonAccounts[ArrTemp[i][15]].columna4;
                        ArrTemp[i][8] = jsonAccounts[ArrTemp[i][15]].columna1;
                        ArrTemp[i][9] = jsonAccounts[ArrTemp[i][15]].columna2;
                    }
                } else {
        ...
        function ObtieneTransacciones(period_aux) {
            ...
            var periodFilterFROM = search.createFilter({
                name: 'formulanumeric',
                formula: 'CASE WHEN {postingperiod.id} IN (' + period_aux + ') THEN 1 ELSE 0 END',
                operator: search.Operator.EQUALTO,
                values: [1]
            });
            savedsearch.filters.push(periodFilterFROM);
