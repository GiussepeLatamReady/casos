...
define(["N/record", "N/runtime", "N/file", "N/search",
        "N/log", "N/config", "N/encode", "N/format",
        "./CO_Library_Mensual/LMRY_CO_Reportes_LBRY_V2.0.js", "/SuiteBundles/Bundle 37714/Latam_Library/LMRY_LibraryReport_LBRY_V2.js"
    ],
    ...
    function(recordModulo, runtime, fileModulo, search, log,
        ...
        function execute(context) {
            ...
            log.debug('ARRACCOUNTS', ArrAccounts);
            log.debug('ARRACCOUNTS LENGHT', ArrAccounts.length);
            ...
            log.debug('arrSALDOANTERIOR', ArrSaldoAnterior.length);
            ...
            log.debug('arrFINAAAL antes de agreupar y ordenar ', ArrFinal);
            log.debug('arrFINAAAL antes de agreupar y ordenar length', ArrFinal.length);
            ArrFinal = AgruparPorPucs(ArrFinal);
            ArrFinal = OrdenarPorPucs(ArrFinal);
            log.debug('arrFINAAAL despues de agreupar y despus de ordenar ', ArrFinal);
            log.debug('arrFINAAAL length', ArrFinal.length);
            ...
            //log.debug('arrFINAAAL despues de Agrupar 6,4,2,1 digits', ArrFinal);
            log.debug('arrFINAAAL length despues de Agrupar 6,4,2,1 digits', ArrFinal.length);
        ...
        function GenerarReporte(ArrMontosFinal) {
            ...
            for (var i = 0; i < ArrMontosFinal.length; i++) {
                ...
                if ((Math.abs(ArrMontosFinal[i][2]) != 0 || Math.abs(ArrMontosFinal[i][3]) != 0 ||
                    ...
                    Math.abs(ArrMontosFinal[i][6]) != 0 || Math.abs(ArrMontosFinal[i][7]) != 0) && Math.abs(saldo_actual) != 0) {
                    contador += 1;
                }
            }
            ...
            if (contador != 0) {
                ...
                for (var i = 0; i < ArrMontosFinal.length; i++) {
                    ...
                    if ((Math.abs(ArrMontosFinal[i][2]) != 0 || Math.abs(ArrMontosFinal[i][3]) != 0 ||
                        ...
                        //Documento
                        if (paramPUC == 'T' || paramPUC == true) {
                            if (ArrMontosFinal[i][0].length != 8) {
                                xlsString += '<Cell></Cell>';
                            } else {
                                xlsString += '<Cell><Data ss:Type="String">' + ArrMontosFinal[i][16] + '</Data></Cell>';
                            }
                        } else {
                            if (ArrMontosFinal[i][0].length != 6) {
                                xlsString += '<Cell></Cell>';
                            } else {
                        ...
                        if (ArrMontosFinal[i][0].length == 1 || ArrMontosFinal[i][0].length == 2 || ArrMontosFinal[i][0].length == 4 || ArrMontosFinal[i][0].length == 6 || ArrMontosFinal[i][0].length == 8) {
                            ...
                            var saldo_mov = redondear(Math.abs(ArrMontosFinal[i][4]) - Math.abs(ArrMontosFinal[i][5]));
                            var saldo_actual = redondear(Math.abs(ArrMontosFinal[i][2]) + Math.abs(ArrMontosFinal[i][4]) - Math.abs(ArrMontosFinal[i][3]) - Math.abs(ArrMontosFinal[i][5]));
                            if (saldo_antes < 0) {
                                //Haber Antes
                            ...
                            if (saldo_mov < 0) {
                                ...
                                xlsString += '<Cell ' + _StyleNum + '><Data ss:Type="Number">' + 0.0 + '</Data></Cell>';
                                //Debe mov
                                xlsString += '<Cell ' + _StyleNum + '><Data ss:Type="Number">' + Math.abs(saldo_mov) + '</Data></Cell>';
                            } else {
                                if (saldo_mov > 0) {
                                    //Haber mov
                                    xlsString += '<Cell ' + _StyleNum + '><Data ss:Type="Number">' + Math.abs(saldo_mov) + '</Data></Cell>';
        ...
        function agregarArregloSeisDigitos(ArrMontosFinal) {
            ...
            //log.debug('Array_cuentas', array_cuentas)
            ...
            log.debug('PASA AL CUATRO DIGITOS 2')
        ...
        function agregarArregloUnDigito2(ArrMontosFinal) {
            ...
            log.debug('arrMontos FINAL 1', ArrMontosFinal)
                //calcular montos de cuentas
            for (var i = 0; i < array_cuentas.length; i++) {
                for (var j = 0; j < ArrMontosFinal.length; j++) {
            ...
            log.debug('NO CREO QUE LLEGUE A QUI')
            return ArrMontosFinal;
        }
        ...
        function AgruparPorPucs(ArrTemp) {
            if (ArrTemp.length != 0) {
                ...
                //log.debug('a veer ArrReturn ', ArrReturn)
                for (var i = 1; i < ArrTemp.length; i++) {
                    var intLength = ArrReturn.length; //? 1, 2, 3
                    for (var j = 0; j < intLength; j++) {
                        //! compara el primero con el segundo (PUC 6 && type+tranID)
                        if (ArrTemp[i][0] == ArrReturn[j][0] && ArrTemp[i][14] == ArrReturn[j][14]) {
