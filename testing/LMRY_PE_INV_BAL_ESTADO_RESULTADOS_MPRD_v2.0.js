 ...
 */
 ...
 define(['N/record', 'N/runtime', 'N/file', 'N/encode', 'N/search', 'N/format', 'N/log', 'N/config', "/SuiteBundles/Bundle 37714/Latam_Library/LMRY_LibraryReport_LBRY_V2.js"],
    function(record, runtime, file, encode, search, format, log, config, libreria) {
        ...
        function getInputData() {
            try {
                ...
                if (hasMultibookFeature || hasMultibookFeature == 'T') {
                    ...
                    VerificarExistenciaCuenta();  
                    for (var i = 0; i < arrAccountingContext.length; i++) {
                        for (var j = 0; j < arrVeriCuenta.length; j++) {
                            //NUMBER = NUEVO CAMPO ACOCUNTING CONTEXT
                            if (arrAccountingContext[i][1] == arrVeriCuenta[j][4]) {
                                arrAccountingContext[i][5] = arrVeriCuenta[j][0];
                                arrAccountingContext[i][6] = arrVeriCuenta[j][5];
                                arrAccountingContext[i][7] = arrVeriCuenta[j][6];
        ...
        function summarize(context) {
            try {
                ...
                if (errores.length > 0) {
                    ...
                    if (catalogCode) {
                        getFinancialStatementCodes(catalogCode, transactionJson);
                    } else {
                        noData()
                    }
                }
        ...
        function getFinancialStatementCodes(catalogCode, transactionJson) {
            ...
            var newSearch = search.create({
                ...
                [
                    ["isinactive", 'is', 'F'],
                    "AND",
                    ["custrecord_lmry_pe_rubro_esta_finan_cat.custrecord_lmry_pe_cata_esta_fina_codigo","is", catalogCode],
                    "AND",
            ...
            for (var i = 0; i < objResult.length; i++) {
                ...
                // 4. Monto
                auxArray[4] = Number(transactionJson[catalogCode + '|' + auxArray[0]] ) || 0;
            ...
            for (var codeId in codesJson) {
                ...
                if(codesJson[codeId][4] != 0){
                    fileContent += anio + '1231' + '|' + catalogCode + '|' + codesJson[codeId][1] + '|' + codesJson[codeId][4] + '|1|\r\n';
                }
            }
        ...
        function getTransactions(Adjustment) {
            ...
            for(var j=0;j<ArrAccountingContextVerif.length;j++){    
                for(var i=0;i<accountsIdArray.length;i++){
                        if(accountsIdArray[i] == ArrAccountingContextVerif[j][4]){
                            accountsJson[accountsIdArray[i]]['CodigoCatalogo'] = ArrAccountingContextVerif[j][6];
                            accountsJson[accountsIdArray[i]]['CodigoRubroEstadoFinanciero'] = ArrAccountingContextVerif[j][7];
                        }
                }    
            }  
...
/*                     logRecord.setValue({
        ...
        function getFileName(fileNumber) {
            ...
            if (hasMultibookFeature) {
                if (fileNumber > 1) {
                    name = 'LE' + companyRuc + anio + mes + dia + '032000' + '07' + '2' + '1' + currency + '1' + '_' + paramMultibook + '_' + count;
                } else {
                    name = 'LE' + companyRuc + anio + mes + dia + '032000' + '07' + '2' + '1' + currency + '1' + '_' + paramMultibook;
                }
            } else {
                if (fileNumber > 1) {
        ...
        function ObtieneAccountingContext() {
            try {
                //0. NUMBER
                //1. NUMBER LOCALIZATION
                //2. NUMBER + NAME LOCALIZATION
                //3. LIBRO
                // Control de Memoria
                var intDMaxReg = 1000;
                var intDMinReg = 0;
                ...
                // No debe hacer doble filtro
/*                 var catalog = search.createFilter({
                    name: 'formulatext',
                    operator: search.Operator.ISNOTEMPTY,
                    formula: '{custrecord_lmry_pe_catalog_esta_finan}'
                });
                savedsearch.filters.push(catalog);

                var financiero = search.createFilter({
                    name: 'formulatext',
                    operator: search.Operator.ISNOTEMPTY,
                    formula: '{custrecord_lmry_rubro_estado_financiero}'
                });
                savedsearch.filters.push(financiero); */
                ...
                log.error('[Error en Busqueda Accounting Context]', e);
            }
        }
        
        function VerificarExistenciaCuenta() {
            // Control de Memoria
            var intDMaxReg = 1000;
            var intDMinReg = 0;
            // Exedio las unidades
            ...
            if (hasSubsidiariesFeature) {
                var accounts_total = search.create({
                    ...
                    filters: [
                        ...
                        "AND", ["subsidiary", "is", paramSubsidiary]
                    ],
                    columns: [
                        // COLUMNA 0
                        search.createColumn({
                        ...
                        search.createColumn({
                            name: 'formulatext',
                            formula: '{custrecord_lmry_pe_catalog_esta_finan.custrecord_lmry_pe_cata_esta_fina_codigo}'
                        }),

                        search.createColumn({
                            name: 'formulatext',
                            formula: '{custrecord_lmry_rubro_estado_financiero.id}'
                        })
                    ]
                });
            } else {
            ...
            var searchresult_account = accounts_total.run();

            while (!DbolStop) {
                var objResult = searchresult_account.getRange(intDMinReg, intDMaxReg);
                if (objResult != null) {
                    var intLength = objResult.length;
                    ...
                    for (var i = 0; i < intLength; i++) {
                        ...
                        if (objResult[i].getValue(columns[0]) != null)
                            arrVerifAccount[0] = objResult[i].getValue(columns[0]);
                        else
                            arrVerifAccount[0] = '';

                        //1. FormulaText
                        if (objResult[i].getValue(columns[1]) != null)
                            arrVerifAccount[1] = objResult[i].getValue(columns[1]);
                        else
                            arrVerifAccount[1] = '';

                        //2. Name
                        if (objResult[i].getValue(columns[2]) != null)
                            arrVerifAccount[2] = objResult[i].getValue(columns[2]);
                        else
                            arrVerifAccount[2] = '';

                        //3. Number
                        if (objResult[i].getValue(columns[3]) != null)
                            arrVerifAccount[3] = objResult[i].getValue(columns[3]);
                        else
                            arrVerifAccount[3] = '';

                        // Number Fijooo
                        if (objResult[i].getValue(columns[4]) != null)
                            arrVerifAccount[4] = objResult[i].getValue(columns[4]);
                        else
                            arrVerifAccount[4] = '';

                        // Catalog code
                        if (objResult[i].getValue(columns[5]) != null)
                            arrVerifAccount[5] = objResult[i].getValue(columns[5]);
                        else
                            arrVerifAccount[5] = '';
                        
                        if (objResult[i].getValue(columns[6]) != null)
                            arrVerifAccount[6] = objResult[i].getValue(columns[6]);
                        else
                            arrVerifAccount[6] = '';
                        
                        arrVeriCuenta[_contPer] = arrVerifAccount;
                        _contPer++;
