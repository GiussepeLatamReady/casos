  ...
  function (recordModulo, runtime, fileModulo, search, format, log, libreria) {
    ...
    function execute(context) {
      try {
        ...
        log.error("[ execute]", error + "Line:" + error.lineNumber);
      }
    }
    ...
    function getParametersAndFeatures() {
      var INFO = objContext.getParameter({
        name: 'custscript_smc_pe_cte_para_global'
      });
    ...
    function sortTransactions() {
      ...
      log.debug("ArrSaldoInic",ArrSaldoInic.length);
      log.debug("ArrCtaCte",ArrCtaCte.length);
      log.debug("ArrSaldoFinal",ArrSaldoFinal.length);
    ...
    function ObtieneCajaBancoPC() {
      ...
      log.debug("ArrPlanCta",ArrPlanCta)
    }
    ...
    function GeneraLibroCajaBanco() {
      ...
      log.debug("arrSaldoInicial General",arrSaldoInicial);
      log.debug("arrMovimiento General",arrMovimiento.length);
      log.debug("arrSaldoFinal General",arrSaldoFinal);
      for (var x = 0; x <= arrSaldoInicial.length - 2; x++) {
        ...
        log.debug("arrSaldoInicial",(arrSaldoInicial[x]).substring(_ini, _len));
        var contMov = 0;
        ...
        for (var xii = 0; xii <= arrSaldoFinal.length - 2; xii++) {
          ...
          if (_CtaCont == arrLineaSF[0]) {
            ...
            log.debug("arrSaldoFinal",(arrSaldoFinal[xii]).substring(_ini, _len));
      ...
      while(i<ArrTemp.length-1){
        ...
        if (t <= file_size) {
          ...
          //log.debug("Content File Final", strName);
          savefile();
    ...
    function savefile() {
      ...
      if (FolderId != '' && FolderId != null) {
        ...
        log.debug("Url File Txt", urlfile);
