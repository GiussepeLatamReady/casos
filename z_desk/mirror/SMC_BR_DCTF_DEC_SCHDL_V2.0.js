 ...
 */
 ...
 "N/log", "N/config", "N/task", "/SuiteBundles/Bundle 37714/Latam_Library/LMRY_LibraryReport_LBRY_V2.js",
...
function(recordModulo, runtime, fileModulo, email, search, format, log,
 ...
 var zip = null;
 var cef = null;
 var phone_complete = null;
 var phone = null;
 var ddd_phone = null;
 var fax_complete = null;
 var fax = null;
 var ddd_fax = null;
 ...
 function execute(context) {
   try {
     ...
     CargarR11(pagosTotales, 'R11'); //formatea la data de pagos a la estructura R11
     ArrJournalR11 = agruparPagos(ArrJournalR11);
     ArrR11Nomina = agruparPagos(ArrR11Nomina);
     formatLinesR11(EPayLines); //formate la data de pagos de EPayment(Pagos y Deducciones)
     ...
     LineasR12 = CargarLineasR12_R14_R15(ArrJournalR12, 'R12');
     var ArrJournalR14 = CargarJournals('R14');
     LineasR14 = CargarLineasR12_R14_R15(ArrJournalR14, 'R14');
     LineasR14 = agruparR14(LineasR14);
     var ArrJournalR15 = CargarJournals('R15');
     LineasR15 = CargarLineasR12_R14_R15(ArrJournalR15, 'R15');
     /********************* CARGADO DE DEDUCCIONES (R16) *******************/
     var lineasDeducciones = CargarJournalR11(31); //ID Concepto: Ficha Deduçao DARF
     CargarR11(lineasDeducciones, 'R16');
     LineasR16 = agruparPagos(LineasR16);
     LineasR16 = LineasR16.concat(EPayLinesDeduc);
 ...
 function getPeriodData(){
       ...
       if (result.length != 0) {
         for (var i = 0; i < result.length; i++) {
           periodField = {
             ...
             startdate: result[i].getValue('custrecord_lmry_taxdate_ini')
           };
         }
       } else {
   ...
   if (periodField != null) {
     //Period EndDate
     periodenddate = periodField.enddate;
     //Nuevo Formato Fecha
     ...
     mes_date = parsedEnddate.getMonth() + 1;
     anio_date = parsedEnddate.getFullYear();
     dia_date = parsedEnddate.getDate();
     ...
     //Period Startdate
     var periodfirstdate = periodField.startdate;
     //Nuevo Formato Fecha
 ...
 function definirPeriodoApuracao(periodicidad, campo_11) {
   ...
   if (periodicidad == 'A') {
     ...
     json.campo9 = anio_date;
     json.campo10 = mes_date;
     json.campo11 = completar(2, campo_11, '0', true);
   }
 ...
 function obtenerDatosCreditFile() {
   ...
   var busq_creditFile = search.create({
     ...
     filters: [
       ...
       "AND", ["custrecord_lmry_br_credit_feature", "anyof", param_Feature]
     ],
 ...
 function CargarJournalR11(concepto) {
   ...
   while (!DbolStop) {
     ...
     if (objResult != null) {
       ...
       for (var i = 0; i < intLength; i++) {
         ...
         if (arrAuxiliar[5] != 0 && conceptoTrnsac == concepto) {
           if (arrAuxiliar[2] == '04' || arrAuxiliar[2] == '09') { //si es impuesto importacion (IOF  o CIDE)
             if (concepto == 5) {//Pagos de Impuesto
       ...
       log.debug('Lineas Journal de Concepto: ' + concepto, matrizData);
 ...
 function CargarJournals(registro) {
   ...
   while (!DbolStop) {
     ...
     if (objResult != null) {
       ...
       for (var i = 0; i < intLength; i++) {
         ...
         if (arrAuxiliar[4] != 0) {
           ...
           if (registro == 'R12') {
             ...
             //6. JSON R15
             var jsonR15 = objResult[i].getValue(columns[10]);
             if (jsonR15 != null && jsonR15 != '' && jsonR15 != '- None -') {
               log.debug('jsonR15', jsonR15);
               arrAuxiliar[6] = JSON.parse(jsonR15);
             } else {
               arrAuxiliar[6] = '';
             }
           } else {
       ...
       log.debug('Journals ' + registro, dataJournal);
       intDMinReg = intDMaxReg;
       intDMaxReg += 1000;
     } else {
       DbolStop = true;
 ...
 function CargarLineasR12_R14_R15(arrayData, tiporegistro) {
   var lineasGeneral = [];
   for (var i = 0; i < arrayData.length; i++) {
     var lineatemp = [];
     //0. Tipo
     ...
     } else if (tiporegistro == 'R15') {
       var jsonR15 = arrayData[i][6];
       //14. Numero do Parcelamento ou Processo
       if (jsonR15.formPedido == '1') {
         lineatemp[14] = completar(17, ValidaGuion(jsonR15.nroProceso), '0', true);
       } else {
         lineatemp[14] = completar(23, ValidaGuion(jsonR15.nroProceso), '0', true);
       }
       //15. Valor Parcelado do Debito
       lineatemp[15] = completar(14, ValidaGuion((montoDebit).toFixed(2)), '0', true);
       //16. Formalização do Pedido
       jsonR15 == '' ? lineatemp[16] = '' : lineatemp[16] = jsonR15.formPedido;
     }
     /* 17/30 Reservado */
     lineatemp.push('          ');
   ...
   log.debug('Lineas Journal ' + tiporegistro, lineasGeneral);
 ...
 function formatLinesR11(arrayData, registro) {
   ...
   for (var i = 0; i < arrayData.length; i++) {
     ...
     r11Columna[16] = '0000';
     if (arrayData[i][9] == 31) {//R16
       if (r11Columna[6] == '933101') {
         r11Columna[16] = '9438';
       } else {
         r11Columna[16] = completar(4, arrayData[i][11], '0', true);
       }
     }else{
       if (arrayData[i][11] != null && arrayData[i][11] != '') {
         r11Columna[16] = completar(4, arrayData[i][11], '0', true);
       }
     }
     ...
     //22. Valor
     if (arrayData[i][9] == 31) {//o Deduzido (R16) - Valor principal
       var montoTotal = (redondear(arrayData[i][4])).toFixed(2);
     }else{//Pago do Débito (sumatoria principal + multas + juros)
     ...
     } else if (arrayData[i][9] == 25) {
       EPayLinesNomina.push(r11Columna);
     } else {
       EPayLinesDeduc.push(r11Columna); //Deducciones R16
     }
   }

 }

 function CargarR11(arrTotal, registro) {
   for (var i = 0; i < arrTotal.length; i++) {
     ...
     if (arrTotal[i][9] == 25) { //PARA CONCEPTO FOHLA DE PAGAMENTO
     ...
     if (arrTotal[i][9] == 25) { //PARA CONCEPTO FOHLA DE PAGAMENTO
       if (r11Columna[7] == 'D') {
         r11Columna[14] = r11Columna[10] + mes_date + anio_date;
       } else {
         r11Columna[14] = dia_date + mes_date + anio_date;
       }
     } else {
     ...
     //22. Valor
     if (arrTotal[i][9] == 31) {//Deduzido
       r11Columna[22] = r11Columna[19];
     }else{//pago do Débito
     ...
     } else if (arrTotal[i][9] == 25) {
       ArrR11Nomina.push(r11Columna);
     } else {
       LineasR16.push(r11Columna);
     }

   }
   log.debug('ArrJournalR11 generado total', ArrJournalR11);
   log.debug('ArrR11Nomina generado total', ArrR11Nomina);
   log.debug('LineasR16 generado total', LineasR16);
 }
 ...
 function GenerarArreglos() {
   ...
   log.debug('valor d arrreglo de Ventas - INventario', ArrSetupDCTF_Sales_Inv);
   ...
   log.debug('vaor de arreglo de Journal IOF', ArrIOFJournal);
   log.debug('vaor de arreglo de Journal CIDE', ArrCIDEJournal);
   log.debug('vaor de arreglo de Bill Pay IOF', ArrIOFBillPay);
   log.debug('vaor de arreglo de Bill Pay CIDE', ArrCIDEBillPay);
   log.debug('vaor de arreglo de Bill Pay EPayment', EPayLines);
 }
 ...
 function CargaDatosRegistrales() {
   ...
   //DDD do Telefone
   var columna15 = ValidaGuion(ddd_phone);
   columna15 = completar(4, columna15, ' ', false);
   //Telefone
   var columna16 = ValidaGuion(phone);
   columna16 = completar(9, columna16, ' ', false);
   //DDD do Fax
   var columna17 = ValidaGuion(ddd_fax);
   columna17 = completar(4, columna17, ' ', false);
   //Fax
   var columna18 = ValidaGuion(fax);
 ...
 function CargaDatosResponsable() {
   ...
   var phone_representante = id_employee_representante.phone.replace('(','/').replace(')','/').split('/');
   if(phone_representante[0] != ''){
     var phone_r = phone_representante[0];
     var dddphone_r = '';
   }else{
     var phone_r = phone_representante[2];
     var dddphone_r = phone_representante[1];
   }
   var fax_representante = id_employee_representante.fax.replace('(','/').replace(')','/').split('/');
   if(fax_representante[0] != ''){
     var fax_r = fax_representante[0];
     var dddfax_r = '';
   }else{
     var fax_r = fax_representante[2];
     var dddfax_r = fax_representante[1];
   }
   ...
   //log.debug('columna7', columna7);
   columna7 = ValidaGuion(columna7);
   columna7 = completar(11, columna7, ' ', false);
   //DDD do Telefone - Representante
   var columna8 = dddphone_r;
   columna8 = ValidaGuion(columna8);
   columna8 = completar(4, columna8, ' ', false);
   log.debug('columna8', phone_representante);
   //Telefone - Representante
   var columna9 = phone_r;
   log.debug('columna9', columna9);
   columna9 = ValidaGuion(columna9);
   columna9 = completar(9, columna9, ' ', false);
   //Ramal - Representante
   var columna10 = '';
   log.debug('columna10', columna10);
   columna10 = completar(5, columna10, ' ', false);
   //DDD do Fax - Representante
   var columna11 = dddfax_r;
   columna11 = completar(4, ValidaGuion(columna11), ' ', false);
   log.debug('columna11', fax_representante);
   //Fax - Representante
   var columna12 = fax_r;
   log.debug('columna12', columna12);
   ...
   log.debug('columna13', columna13);
   columna13 = completar(40, columna13, ' ', false);

   log.debug('name_responsable', name_responsable);
   ...
   var phone_responsavel = id_employee_responsable.phone.replace('(','/').replace(')','/').split('/');
   if(phone_responsavel[0] != ''){
     var phone_rl = phone_responsavel[0];
     var dddphone_rl = '';
   }else{
     var phone_rl = phone_responsavel[2];
     var dddphone_rl = phone_responsavel[1];
   }
   var fax_responsavel = id_employee_responsable.fax.replace('(','/').replace(')','/').split('/');
   if(fax_responsavel[0] != ''){
     var fax_rl = fax_responsavel[0];
     var dddfax_rl = '';
   }else{
     var fax_rl = fax_responsavel[2];
     var dddfax_rl = fax_responsavel[1];
   }
   ...
   log.debug('columna17', columna17);
   columna17 = completar(2, columna17, ' ', false);
   //DDD do Telefone - Responsável
   var columna18 = dddphone_rl;
   columna18 = ValidaGuion(columna18);
   columna18 = completar(4, columna18, ' ', false);
   log.debug('columna18', phone_responsavel);
   //Telefone - Responsável
   var columna19 = phone_rl;
   log.debug('columna19', columna19);
   ...
   var columna21 = dddfax_rl;
   columna21 = completar(4, ValidaGuion(columna21), ' ', false);
   log.debug('columna21', fax_responsavel);
   //Fax - Responsável
   var columna22 = fax_rl;
 ...
 function CargarPayRoll() {
   ...
   if (intLength > 0) {
     for (var i = 0; i < intLength; i++) {
       ...
       log.debug('monto_payroll', monto_payroll);
       ...
       log.debug('name_type_concepto', name_type_concepto);
     }
   }
 }
 ...
 function CargarPayRollCPRB() {
   ...
   if (intLength > 0) {
     for (var i = 0; i < intLength; i++) {
       ...
       log.debug('monto_payroll_CPRB', monto_payroll_CPRB);
     }
   }
 }
 ...
 function armarR10(arrayLineas, cnpj_subsi) {
   ...
   for (var i = 0; i < arrayLineas.length; i++) {
     ...
     if (columna15 != '00000000000000') {
       ...
       lineasR10 += ObtenerSubRegistro(columna6, columna7, columna8, columna12, LineasR15);
       lineasR10 += ObtenerSubRegistro(columna6, columna7, columna8, columna12, LineasR16);
     }
 ...
 function ObtenerParametrosYFeatures() {
   ...
   param_RecorID = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_recorid'
   });
   param_Periodo = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_periodo'
   });
   param_Subsi = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_subsi'
   });
   param_Multi = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_multi'
   });
   param_Type_Decla = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_decla'
   });
   param_Num_Recti = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_recti'
   });
   param_archivo = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_archivo'
   });
   param_Lucro_Conta = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_lucro_conta'
   });
   param_Monto_Adicional = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_monto_adic'
   });
   param_Monto_Excluyente = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_monto_excluy'
   });
   param_Feature = objContext.getParameter({
     name: 'custscript_smc_br_dctf_dec_featureid'
   });
   ...
   var subsi_temp = search.lookupFields({
     type: search.Type.SUBSIDIARY,
     id: id_subsi,
     columns: ['taxidnum', 'custrecord_lmry_br_uf', 'address.zip', 'custrecord_lmry_br_cef', 'address.address1', 'address.phone', 'custrecord_lmry_br_porc_economic_activiy', 'custrecord_lmry_br_regimen_pis_confis', 'address.custrecord_lmry_addr_prov_acronym', 'address.custrecord_lmry_address_number', 'address.custrecord_lmry_addr_reference', 'address.address2', 'address.custrecord_lmry_addr_city', 'custrecord_lmry_email_subsidiaria','fax']
   });
   ...
   phone_complete = subsi_temp['address.phone'].replace('(','/').replace(')','/').split('/');
   if(phone_complete[0] != ''){
     phone = phone_complete[0];
     ddd_phone = '';
   }else{
     phone = phone_complete[2];
     ddd_phone = phone_complete[1];
   }
   fax_complete = subsi_temp['fax'].replace('(','/').replace(')','/').split('/');
   if(fax_complete[0] != ''){
     fax = fax_complete[0];
     ddd_fax = '';
   }else{
     fax = fax_complete[2];
     ddd_fax = fax_complete[1];
   }
