 ...
 */
 ...
 "N/log", "N/config", "N/task", 'N/encode', "/SuiteBundles/Bundle 37714/Latam_Library/LMRY_LibraryReport_LBRY_V2.js",
...
function(recordModulo, runtime, fileModulo, email, search, format, log,
 ...
 var featTaxSpecial = null;
 var featureTaxCalendars = null;
 var featureMultipCalendars = null;
 ...
 var periodname = null;
 //Datos de subsidiaria
 var subsiFiscalCalendar = '';
 var subsiTaxFiscalCalendar = '';
 ...
 var ArrJournalR15 = new Array();
 var LineasR15 = new Array();
 ...
 var ArrJournalR16 = new Array();
 var ArrEPayLinesDeducc = new Array();
 var LineasR16 = new Array();
 ...
 function execute(context) {
   try {
     ...
     formatEPayLines(ArrEPayLines); //formatea y distribuye la data de Journals e EPayments - (R11/R16)
     ...
     //R15
     ArrJournalR15 = obtenerJournalsR12_R14('R15');
     //R16
     ArrJournalR16 = ObtenerDataPagosTributos(31); //Deducciones R16
     ArrJournalR16 = agruparR11(ArrJournalR16);
     ArrJournalR16 = ArrJournalR16.concat(ArrEPayLinesDeducc);
     log.debug('ArrJournalR16', ArrJournalR16);
 ...
 function ObtenerConfigFeature(id_subsi, id_feature) {
   ...
   licenses = libFeature.getLicenses(id_subsi);
 ...
 function getPeriodData(){
   featAccountingSpecial = ObtenerConfigFeature(param_Subsi, 599);
   featureTaxCalendars = ObtenerConfigFeature(param_Subsi, 681);
   featTaxSpecial = ObtenerConfigFeature(param_Subsi, 682);
   var periodField = null;
   log.debug('featAccountingSpecial', featAccountingSpecial);

   if (featureTaxCalendars || featureTaxCalendars == 'T') {

     if (featTaxSpecial || featTaxSpecial == 'T') {
       log.debug('2');
       var SearchPeriodSpecial = search.create({
         ...
         columns: ["custrecord_lmry_taxdate_ini", 'name', 'custrecord_lmry_taxdate_fin']
       });
       ...
       if (featureMultipCalendars || featureMultipCalendars == 'T') {
         var fiscalCalendarFilter = search.createFilter({
           ...
           values: subsiTaxFiscalCalendar
         });
         SearchPeriodSpecial.filters.push(fiscalCalendarFilter);
       }

       var result = SearchPeriodSpecial.run().getRange(0, 1);
       log.debug('result special accounting', result);
       if (result.length != 0) {
         for (var i = 0; i < result.length; i++) {
           periodField = {
             periodname: result[i].getValue('name'),
             enddate: result[i].getValue('custrecord_lmry_taxdate_fin')
           };
         }
       } else {
         //Esto ya se valida por suitelet
         log.debug('Alerta', 'No se configuró periodo en Special Tax Period');
       }
     ...
     if (featAccountingSpecial || featAccountingSpecial == 'T') {
       ...
       var specialAccountingSearch = search.create({
         ...
         filters: [
           ['isinactive', 'is', 'F'], "AND", ["custrecord_lmry_accounting_period", "anyof", param_Periodo]
         ],
         columns: [
           search.createColumn({
             name: "name",
       ...
       if (featureMultipCalendars || featureMultipCalendars == 'T') {
         var fiscalCalendarFilter = search.createFilter({
           ...
           operator: search.Operator.IS,
           values: subsiFiscalCalendar
         });
         specialAccountingSearch.filters.push(fiscalCalendarFilter);
       }
       ...
       periodField = search.lookupFields({
   ...
   log.debug('periodField', periodField);

   if (periodField != null) {
     //Period EndDate
     var periodenddate = periodField.enddate;
     //Nuevo Formato Fecha
     ...
     //Period Name
     periodname = periodField.periodname;
 ...
 function GenerarArreglos() {
   ...
   log.debug('vaor de arreglo de Bill Pay IOF', ArrIOFBillPay);
   log.debug('vaor de arreglo de Bill Pay CIDE', ArrCIDEBillPay);
   log.debug('vaor de arreglo de Bill Pay E Payment', ArrEPayLines);
 }

 //presumido ---  estimativo
 ...
 function GenerarHojaPagosJournal(VectorPagosTributosParam) {
   if (language == 'es') {
     ...
     var ImpR11 = 'Datos relacionado a Impuestos Pagados - R11';
   } else if (language == 'pt') {
     var nameHojaJournal = 'Impostos pagos';
     var ImpR11 = 'Dados relacionados a impostos pagos - R11';
   } else {
     var nameHojaJournal = 'Paid taxes';
     var ImpR11 = 'Data related to Taxes Paid - R11';
   }
   ...
   if (VectorPagosTributosParam.length != 0) {
     ...
     strSheetJournals += CargarTituloHojaExcel(6, ImpR11);
     strSheetJournals += '<Row>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["numDoc"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["TDoc"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["FecPag"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["FecVen"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Trib"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["CodTr"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Ingr"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Perioc"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValPri"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValMul"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValInt"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValPag"][language] + '</Data></Cell>' +
       '</Row>';
 ...
 function GenerarHojaDeducciones(arrData) {
   if (language == 'es') {
     var nameHojaJournal = 'Deducciones con DARF';
     var title = 'Datos relacionadoa a Deducciones con DARF - R16';
   } else if (language == 'pt') {
     var nameHojaJournal = 'Deduções com DARF';
     var title = 'Dados relacionados a Deduções com DARF - R16';
   } else {
     var nameHojaJournal = 'Deductions with DARF';
     var title = 'Data related to Deductions with DARF - R16';
   }
   var arrayOrdenado = [];
   var strSheetJournals = '';
   //Aqui debe ponerse el vector con journals q si tienen deuda de tributo
   if (arrData.length != 0) {
     ...
     strSheetJournals += CargarTituloHojaExcel(6, title);
     strSheetJournals += '<Row>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["numDoc"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["TDoc"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["FecDed"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["FecVen"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Trib"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["CodTr"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Ingr"][language] + ' </Data></Cell>' +
       ...
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValMul"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValInt"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValDed"][language] + '</Data></Cell>' +
       '</Row>';

     log.debug('Todos los Journals sin ordenar', arrData);
     //Ordenar Vector de Journals por Cod de Tributo
     arrayOrdenado = OrdenarJournalsTributo(arrData);
     log.debug('arrayOrdenado', arrayOrdenado);

     for (var i = 0; i < arrayOrdenado.length; i++) {
       strSheetJournals += '<Row>';
       for (var j = 0; j < arrayOrdenado[i].length; j++) {
         strSheetJournals += '<Cell ss:StyleID="s25" ><Data ss:Type="String">' + arrayOrdenado[i][j] + '</Data></Cell>';
       }
       strSheetJournals += '</Row>';
     }
 ...
 function GenerarHojaCompensacionesJournal(lineasJournal) {
   if (language == 'es') {
     ...
     var ImpR12 = 'Datos relacionado a Compensaciones - R12';
   } else if (language == 'pt') {
     var nameHojaJournal = 'Compensação';
     var ImpR12 = 'Dados relacionados a Compensações - R12';
   } else {
     var nameHojaJournal = 'Compensation';
     var ImpR12 = 'Data related to Compensations - R12';
   }
   ...
   if (lineasJournal.length != 0) {
     ...
     strSheetJournals += CargarTituloHojaExcel(4, ImpR12);
     strSheetJournals += '<Row>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["numDoc"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["TDoc"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["FecPag"][language] + ' </Data></Cell>' +
       ...
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Ingr"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Perioc"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValCom"][language] + '</Data></Cell>' +
       '</Row>';
 ...
 function GenerarHojaSuspensionesJournal(lineasJournal) {
   ...
   if (language == 'es') {
     ...
     var ImpR14 = 'Datos relacionado a Suspensiones - R14';
   } else if (language == 'pt') {
     var nameHojaJournal = 'Suspensões';
     var ImpR14 = 'Dados Relacionados a Suspensões - R14';
   } else {
     var nameHojaJournal = 'Suspensions';
     var ImpR14 = 'Suspensions Related Data - R14';
   }
   ...
   if (lineasJournal.length != 0) {
     ...
     strSheetJournals += CargarTituloHojaExcel(5, ImpR14);
     strSheetJournals += '<Row>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["numDoc"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["TDoc"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["FecPag"][language] + ' </Data></Cell>' +
       ...
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Ingr"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Perioc"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValSus"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValPri"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValMul"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValInt"][language] + '</Data></Cell>' +
       '</Row>';
 ...
 function GenerarHojaParcelamentoJournal(lineasJournal) {
   if (language == 'es') {
     var nameHojaJournal = 'Entrega';
     var ImpR15 = 'Datos relacionado a Entrega - R15';
   } else if (language == 'pt') {
     var nameHojaJournal = 'Parcelamento';
     var ImpR15 = 'Dados relacionados a Parcelamento - R15';
   } else {
     var nameHojaJournal = 'Installment';
     var ImpR15 = 'Data related to Installment - R15';
   }
   ...
   if (lineasJournal.length != 0) {
     ...
     strSheetJournals += CargarTituloHojaExcel(4, ImpR15);
     strSheetJournals += '<Row>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["numDoc"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["TDoc"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["FecPag"][language] + ' </Data></Cell>' +
       ...
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Ingr"][language] + ' </Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Perioc"][language] + '</Data></Cell>' +
       '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValParc"][language] + '</Data></Cell>' +
       '</Row>';
 ...
 function SaveFile() {
   ...
   if (FolderId != '' && FolderId != null) {
     ...
     var NameFile = "SMC_"+Name_File() + '.xls';
     // Crea el archivo
 ...
 function CargarPayRoll() {
   ...
   var savedsearch = search.create({
     ...
     filters: [
       ["type", "anyof", "Journal"],
       "AND", ["postingperiod", "is", param_Periodo],
       "AND", ["posting", "is", "T"],
       "AND", ["voided", "is", "F"],
       "AND", ["memorized", "is", "F"],
       "AND", ["formulatext: {custbody_lmry_type_concept.id}", "is", "1"]
     ],
     settings: [
 ...
 function formatEPayLines(arrData) {
   ...
   for (var i = 0; i < arrData.length; i++) {
     ...
     //11 Valor Pago
     if (concepto == 31) {
       var total = Number(montoPrincipal);
     }else{
 ...
 function ObtenerDataPagosTributos(concepto) {
   ...
   //Periodo
   if (featureTaxCalendars || featureTaxCalendars == 'T') {
     var formulaPeriod = "CASE WHEN {custbody_lmry_cl_period.custrecord_lmry_tax_period.id}='" + param_Periodo + "' THEN 1 ELSE 0 END";
   } else {
 ...
 function obtenerJournalsR12_R14(registro) {
   ...
   while (!DbolStop) {
     ...
     if (objResult != null) {
       ...
       log.debug('Journals ' + registro, arrayData);
       intDMinReg = intDMaxReg;
       intDMaxReg += 1000;
     } else {
       DbolStop = true;
 ...
 function obtenerVectorJournalTributo(tributo, receita, periodicidad, arrayData, arrayResult) {
   ...
   var i = 0;
   while (i < long) {
     if (arrayData[i][5] == tributo && arrayData[i][6] == receita && arrayData[i][7] == periodicidad) {
 ...
 function CargarPayRollCPRB() {
   ...
   var savedsearch = search.create({
     ...
     filters: [
       ...
       "AND", ["voided", "is", "F"],
       "AND", ["memorized", "is", "F"],
       "AND", ["formulatext: {custbody_lmry_type_concept.id}", "is", "4"]
     ],
     settings: [
       search.createSetting({
         name: 'consolidationtype',
 ...
 function CargarTituloHojaExcel(nroEspacio, titulo) {
   ...
   strtituloExcel += '<Cell ss:StyleID="s23" ><Data ss:Type="String">' + GLOBAL_LABELS["Perd"][language] + '</Data></Cell>';
 ...
 function CargaDatosDebCredRealEstimativo() {
   ...
   if (language == 'es') {
     var nameHojaJournal = "Impuestos a Pagar";
     var ReoAu = 'Reporte Auditoria de Impuestos adeudados';
   } else if (language == 'pt') {
     var nameHojaJournal = "Impostos a pagar";
     var ReoAu = 'Relatório de Auditoria de impostos devidos';
   } else {
     var nameHojaJournal = "Taxes to Pay";
     var ReoAu = 'Audit Report of Taxes owed';
   }

   strRetenciones_aux += ' <Worksheet ss:Name="' + nameHojaJournal + '">';
   strRetenciones_aux += '<Table>';
   strRetenciones_aux += CargarTituloHojaExcel(3, ReoAu);
   ...
   strRetenciones_aux += '<Row>' +
     ...
     '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Ingr"][language] + ' </Data></Cell>' +
     '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["Perioc"][language] + '</Data></Cell>' +
     '<Cell ss:StyleID="s20"><Data ss:Type="String"> ' + GLOBAL_LABELS["ValDeb"][language] + ' </Data></Cell>' +
     '</Row>';
   ...
   for (var i = 0; i < ArrSetupDCTF_Sales.length; i++) {
     if (ArrSetupDCTF_Sales[i][1] == '07' || ArrSetupDCTF_Sales[i][1] == '06') {
       if (agregarSeparador) {
         colocarSeparador(GLOBAL_LABELS["Vents"][language]);
       ...
       strRetenciones_aux += '</Row>';
       obtenerVectorJournalTributo(ArrSetupDCTF_Sales[i][1], ArrSetupDCTF_Sales[i][3], ArrSetupDCTF_Sales[i][4], VectorPagosTributos, JournalsCoinciR11);
       ...
       obtenerR12_R14Journal(ArrSetupDCTF_Sales[i][1], ArrSetupDCTF_Sales[i][3], ArrSetupDCTF_Sales[i][4], ArrJournalR15, LineasR15);
       obtenerVectorJournalTributo(ArrSetupDCTF_Sales[i][1], ArrSetupDCTF_Sales[i][3], ArrSetupDCTF_Sales[i][4], ArrJournalR16, LineasR16);
     }
   }
   ...
   for (var i = 0; i < ArrSetupDCTF_Sales_Inv.length; i++) {
     if (ArrSetupDCTF_Sales_Inv[i][1] == '06' || ArrSetupDCTF_Sales_Inv[i][1] == '07' || ArrSetupDCTF_Sales_Inv[i][1] == '03') {
       if (agregarSeparador) {
         colocarSeparador(GLOBAL_LABELS["VentInv"][language]);
         agregarSeparador = false;
       }
       strRetenciones_aux += '<Row>';
       //NUMERO DOC
       ...
       strRetenciones_aux += '</Row>';
       obtenerVectorJournalTributo(ArrSetupDCTF_Sales_Inv[i][1], ArrSetupDCTF_Sales_Inv[i][3], ArrSetupDCTF_Sales_Inv[i][4], VectorPagosTributos, JournalsCoinciR11);
       ...
       obtenerR12_R14Journal(ArrSetupDCTF_Sales_Inv[i][1], ArrSetupDCTF_Sales_Inv[i][3], ArrSetupDCTF_Sales_Inv[i][4], ArrJournalR15, LineasR15);
       obtenerVectorJournalTributo(ArrSetupDCTF_Sales_Inv[i][1], ArrSetupDCTF_Sales_Inv[i][3], ArrSetupDCTF_Sales_Inv[i][4], ArrJournalR16, LineasR16);
     }
   }
   /************************************* CARGAR PURCHASES *****************************************************/
   agregarSeparador = true;
   log.debug('ArrSetupDCTF_Purchases antes', ArrSetupDCTF_Purchases);
   for (var i = 0; i < ArrSetupDCTF_Purchases.length; i++) {
     if (ArrSetupDCTF_Purchases[i][1] == '07' || ArrSetupDCTF_Purchases[i][1] == '06') {
       if (agregarSeparador) {
         colocarSeparador(GLOBAL_LABELS["Comprs"][language]);
         agregarSeparador = false;
       }
       strRetenciones_aux += '<Row>';
       //NUMERO DOC
       ...
       strRetenciones_aux += '</Row>';
       log.debug('ArrSetupDCTF_Purchases', ArrSetupDCTF_Purchases);

       obtenerVectorJournalTributo(ArrSetupDCTF_Purchases[i][1], ArrSetupDCTF_Purchases[i][3], ArrSetupDCTF_Purchases[i][4], VectorPagosTributos, JournalsCoinciR11);
       ...
       obtenerR12_R14Journal(ArrSetupDCTF_Purchases[i][1], ArrSetupDCTF_Purchases[i][3], ArrSetupDCTF_Purchases[i][4], ArrJournalR15, LineasR15);
       obtenerVectorJournalTributo(ArrSetupDCTF_Purchases[i][1], ArrSetupDCTF_Purchases[i][3], ArrSetupDCTF_Purchases[i][4], ArrJournalR16, LineasR16);
     }
   }
   ...
   for (var i = 0; i < ArrSetupDCTF_Purchases_Inv.length; i++) {
     if (ArrSetupDCTF_Purchases_Inv[i][1] == '06' || ArrSetupDCTF_Purchases_Inv[i][1] == '07' || ArrSetupDCTF_Purchases_Inv[i][1] == '03') {
       if (agregarSeparador) {
         colocarSeparador(GLOBAL_LABELS["ComprInv"][language]);
         agregarSeparador = false;
       }
       ...
       strRetenciones_aux += '</Row>';
       obtenerVectorJournalTributo(ArrSetupDCTF_Purchases_Inv[i][1], ArrSetupDCTF_Purchases_Inv[i][3], ArrSetupDCTF_Purchases_Inv[i][4], VectorPagosTributos, JournalsCoinciR11);
       ...
       obtenerR12_R14Journal(ArrSetupDCTF_Purchases_Inv[i][1], ArrSetupDCTF_Purchases_Inv[i][3], ArrSetupDCTF_Purchases_Inv[i][4], ArrJournalR15, LineasR15);
       obtenerVectorJournalTributo(ArrSetupDCTF_Purchases_Inv[i][1], ArrSetupDCTF_Purchases_Inv[i][3], ArrSetupDCTF_Purchases_Inv[i][4], ArrJournalR16, LineasR16);
     }
   }
   ...
   if (param_Lucro_Conta != 0) {
     colocarSeparador(GLOBAL_LABELS["Calcls"][language]);
     ...
     strRetenciones_aux += '</Row>';
     obtenerVectorJournalTributo('01', id_receta_irpj, id_periodicidad_irpj, VectorPagosTributos, JournalsCoinciR11);
     ...
     obtenerR12_R14Journal('01', id_receta_irpj, id_periodicidad_irpj, ArrJournalR15, LineasR15);
     obtenerVectorJournalTributo('01', id_receta_irpj, id_periodicidad_irpj, ArrJournalR16, LineasR16);
   ...
   if (param_Lucro_Conta != 0) {
     ...
     strRetenciones_aux += '</Row>';
     obtenerVectorJournalTributo('05', id_receta_csll, id_periodicidad_csll, VectorPagosTributos, JournalsCoinciR11);
     ...
     obtenerR12_R14Journal('05', id_receta_csll, id_periodicidad_csll, ArrJournalR15, LineasR15);
     obtenerVectorJournalTributo('05', id_receta_csll, id_periodicidad_csll, ArrJournalR16, LineasR16);
   ...
   for (var j = 0; j < Arr_WHT_No_Item.length; j++) {
     if (Arr_WHT_No_Item[j][0] == '02' || Arr_WHT_No_Item[j][0] == '11') {
       if (agregarSeparador) {
         colocarSeparador(GLOBAL_LABELS["Retens"][language]);
         agregarSeparador = false;
       }
       strRetenciones_aux += '<Row>';
       //NUMERO DOC
       ...
       strRetenciones_aux += '</Row>';
       obtenerVectorJournalTributo(Arr_WHT_No_Item[j][0], Arr_WHT_No_Item[j][3], Arr_WHT_No_Item[j][5], VectorPagosTributos, JournalsCoinciR11);
       ...
       obtenerR12_R14Journal(Arr_WHT_No_Item[j][0], Arr_WHT_No_Item[j][3], Arr_WHT_No_Item[j][5], ArrJournalR15, LineasR15);
       obtenerVectorJournalTributo(Arr_WHT_No_Item[j][0], Arr_WHT_No_Item[j][3], Arr_WHT_No_Item[j][5], ArrJournalR16, LineasR16);
   ...
   for (var j = 0; j < Arr_WHT_Item.length; j++) {
     if (Arr_WHT_Item[j][0] == '02') {
       if (agregarSeparador) {
         colocarSeparador(GLOBAL_LABELS["RetenRepLe"][language]);
         agregarSeparador = false;
       }
       strRetenciones_aux += '<Row>';
       //NUMERO DOC
       ...
       strRetenciones_aux += '</Row>';
       obtenerVectorJournalTributo(Arr_WHT_Item[j][0], Arr_WHT_Item[j][3], Arr_WHT_Item[j][5], VectorPagosTributos, JournalsCoinciR11);
       ...
       obtenerR12_R14Journal(Arr_WHT_Item[j][0], Arr_WHT_Item[j][3], Arr_WHT_Item[j][5], ArrJournalR15, LineasR15);
       obtenerVectorJournalTributo(Arr_WHT_Item[j][0], Arr_WHT_Item[j][3], Arr_WHT_Item[j][5], ArrJournalR16, LineasR16);
   ...
   if (Type_concept_journal == '1') {
     if (ArrPayroll.length != 0) {
       colocarSeparador(GLOBAL_LABELS["RetenPay"][language]);
       for (var i = 0; i < ArrPayroll.length; i++) {
         ...
         strRetenciones_aux += '</Row>';
         obtenerVectorJournalTributo(id_tributo_payroll, id_receta_payroll, id_periodicidad_payroll, VectorPagosTributos, JournalsCoinciR11);
         ...
         obtenerR12_R14Journal(id_tributo_payroll, id_receta_payroll, id_periodicidad_payroll, ArrJournalR15, LineasR15);
         obtenerVectorJournalTributo(id_tributo_payroll, id_receta_payroll, id_periodicidad_payroll, ArrJournalR16, LineasR16);
   ...
   if (ArrLineasNomina.length != 0) {
     colocarSeparador(GLOBAL_LABELS["ImpNom"][language]);
   ...
   log.debug('LineasR15', LineasR15);
   strRetenciones_aux += GenerarHojaParcelamentoJournal(LineasR15);
   log.debug('LineasR16', LineasR16);
   strRetenciones_aux += GenerarHojaDeducciones(LineasR16);
 ...
 function CargaDatosDebCredPresumido() {
   ...
   /************************************** CARGAR IMPOSTOS DE NOMINA *********************************************/
   if (ArrLineasNomina.length != 0) {
     colocarSeparador(GLOBAL_LABELS["ImpSobNom"][language]);
     for (var i = 0; i < ArrLineasNomina.length; i++) {
       strRetenciones_aux += '<Row>';
       //NUMERO DOC
       strRetenciones_aux += '<Cell ss:StyleID="s25"><Data ss:Type="String">' + ArrLineasNomina[i][0] + '</Data></Cell>';
 ...
 function agregarLineasImportacion(arrayJournal) {
   ...
   for (var i = 0; i < arrayJournal.length; i++) {
     if (i == 0) {
       colocarSeparador(GLOBAL_LABELS["Import"][language]);
     }
     strRetenciones_aux += '<Row>';
     //NUMERO DOC
     ...
     obtenerVectorJournalTributo(arrayJournal[i][1], arrayJournal[i][3], arrayJournal[i][4], VectorPagosTributos, JournalsCoinciR11);
     ...
     obtenerR12_R14Journal(arrayJournal[i][1], arrayJournal[i][3], arrayJournal[i][4], ArrJournalR15, LineasR15);
     obtenerVectorJournalTributo(arrayJournal[i][1], arrayJournal[i][3], arrayJournal[i][4], ArrJournalR16, LineasR16);
   }
 }
 ...
 function ObtenerDatosSubsidiaria() {
   ...
   if (feature_Subsi) {
     ...
     if (featureMultipCalendars || featureMultipCalendars == 'T') {
       ...
       var subidiaryField = search.lookupFields({
       ...
       subsiFiscalCalendar = {
         id: subidiaryField.fiscalcalendar[0].value,
         nombre: subidiaryField.fiscalcalendar[0].text
       }
       subsiFiscalCalendar = JSON.stringify(subsiFiscalCalendar);

       log.debug('subsiFiscalCalendar', subsiFiscalCalendar);
       subsiTaxFiscalCalendar = {
         id: subidiaryField.taxfiscalcalendar[0].value,
         nombre: subidiaryField.taxfiscalcalendar[0].text
       }

       subsiTaxFiscalCalendar = JSON.stringify(subsiTaxFiscalCalendar);
       log.debug('subsiTaxFiscalCalendar', subsiTaxFiscalCalendar);
     ...
     companyname = subidiaryField.legalname;
     companyruc = subidiaryField.taxidnum;
 ...
 function ObtenerParametrosYFeatures() {
   param_RecordLogID = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_logid'
   });
   param_Periodo = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_periodo'
   });
   param_Subsi = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_subsid'
   });
   param_Multi = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_multibook'
   });
   param_Type_Decla = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_tipo_decla'
   });
   param_Num_Recti = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_num_rec'
   });
   param_Lucro_Conta = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_lucro'
   });
   param_Monto_Adicional = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_monto_adic'
   });
   param_Monto_Excluyente = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_monto_excl'
   });
   param_Feature = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_featureid'
   });
   param_File = objContext.getParameter({
     name: 'custscript_smc_br_dctf_xls_fileid'
   });
 ...
 function getGlobalLabels() {
   var labels = {
     ...
     "ValCom": {
       "es": 'VALOR DE COMPENSACION',
       "pt": 'VALOR DE COMPENSAÇÃO',
       "en": 'COMPENSATION VALUE'
     },
     "ValSus": {
       "es": 'VALOR DE SUSPENSIÓN',
       "pt": 'VALOR DE SUSPENSÃO',
       "en": 'SUSPENSION VALUE'
     },
     "ValParc": {
       "es": 'VALOR DE CUOTA',
     ...
     "VentInv": {
       "es": "VENTAS INVENTARIO",
       "pt": "VENDAS - INVENTÁRIO",
       "en": "INVENTORY SALES"
     },
     "Comprs": {
       "es": "COMPRAS",
     ...
     "ComprInv": {
       "es": "COMPRAS INVENTARIO",
       "pt": "COMPRAS - INVENTÁRIO",
       "en": "PURCHASES INVENTORY"
     },
     "Calcls": {
       "es": "CALCULOS",
       "pt": "CÁLCULOS",
     ...
     "RetenRepLe": {
       "es": "RETENCIÓN REPRESENTANTE LEGAL",
       "pt": "RETENÇÃO DE REPRESENTANTE LEGAL",
       "en": "LEGAL REPRESENTATIVE RETENTION"
     },
     "RetenPay": {
       "es": "RETENCIÓN DE NÓMINA",
       "pt": "RETENÇÃO DE FOLHA DE PAGAMENTO",
       "en": "PAYROLL RETENTION"
     },
     "ImpNom": {
       "es": "IMPUESTOS NOMINA",
       "pt": "IMPOSTO SOBRE OS SALÁRIOS",
       "en": "PAYROLL TAX"
     },
     "ImpSobNom": {
       "es": "IMPUESTOS SOBRE LA NÓMINA",
       "pt": "IMPOSTOS SOBRE OS SALÁRIOS",
       "en": "PAYROLL ON TAXES"
     },
     "Import": {
       "es": "IMPORTACIONES",
