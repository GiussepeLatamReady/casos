 ...
 */
 ...
 function (record, runtime, file, search, format, log, config, task, libreriaLicense, library) {
     ...
     function map(context) {
         try {
             ...
             if (dataInput["isError"] == "T") {
                 ...
                 //paramsubsidi
                 PARAMETERS.RECORDID = 3948;
                 var parametroLog = search.lookupFields({
                    type: 'customrecord_lmry_pe_2016_rpt_genera_log',
                    id: PARAMETERS.RECORDID,
                    columns: ['custrecord_lmry_pe_2016_rg_parametro']
                 });
                 var jsonLog = parametroLog.custrecord_lmry_pe_2016_rg_parametro;
                 var jsonLogParse = JSON.parse(jsonLog);
                 PARAMETERS.SUBSID = jsonLogParse.subsidiary;
         ...
         try {
             ...
             if (arrTransactions.length != 0) {
                 ...
                 while (isRepeat) {
                     ...
                     // log.debug("Archivo Txt", stringTemporal)
     ...
     function getParametersAndFeatures() {
        ...
        var jsonLog = parametroLog.custrecord_lmry_pe_2016_rg_parametro;
        var jsonLogParse = JSON.parse(jsonLog);
        log.debug('jsonparse',jsonLogParse);

        //paramperiodo
        PARAMETERS.PERIOD = jsonLogParse.periodo;

        //paramClosedPeriod
        PARAMETERS.CLOSED_PERIOD = jsonLogParse.period_closing;

        //paramsubsidi
        PARAMETERS.SUBSID = jsonLogParse.subsidiary;

        //paramMultibook
        PARAMETERS.MULTIBOOK = jsonLogParse.multibook;

        //paramTipoExtPeriodo
        PARAMETERS.TYPE_EXT_PERIOD = jsonLogParse.periodo_tipo_extraccion;

        //paramIndicadorOperaciones
        PARAMETERS.INDIC_OPERAC = jsonLogParse.indicador_operaciones;
     ...
     function getCajaBankCtaCte() {
         ...
         if (PARAMETERS.CLOSED_PERIOD == 'T' || PARAMETERS.CLOSED_PERIOD == true) {
             ...
             log.debug("paramFechaIni",paramFechaIni)
             log.debug("paramFechaFin",paramFechaFin)
             if (auxiliar[1] == '12') {
         ...
         if (FEATURES.MULTIBOOK) { //estaba vacio
             ...
             var lineunikey = search.createColumn({
                name: "formulatext",
                formula: "{lineuniquekey}",
                label: "lineunikey"
             })
             savedsearch.columns.push(lineunikey);
         } else {
         ...
         pageData.pageRanges.forEach(function (pageRange) {
             ...
             page.data.forEach(function (result) {
                 ...
                 if (FEATURES.MULTIBOOK) {
                     ...
                     if (result.getValue(columns[26]) != null) {
                        arrTemp[25] = result.getValue(columns[26]);
                     } else {
                        arrTemp[25] = '';
                     }
                 } else {
                     ...
                     if (result.getValue(columns[24]) != null) {
                        arrTemp[25] = result.getValue(columns[24]);
                     } else {
                        arrTemp[25] = '';
                     }
                 }
     ...
     function enterEntity(transaction){
         if (transaction[21] == 'Journal' ||transaction[21] == 'Asiento' || transaction[21] == 'Diario' || transaction[21] == 'PEJrnl') {
     ...
     function callSchedule() {
         ...
         params['custscript_smc_pe_cte_para_global'] = param;
         var RedirecSchdl = task.create({
             taskType: task.TaskType.SCHEDULED_SCRIPT,
             scriptId: 'customscript_smc_pe_cajabanco_cuentacte',
             deploymentId: 'customdeploysmc_pe_cajabanco_cuentacte',
             params: params
         });
         RedirecSchdl.submit();
     ...
     function saveAuxiliarFile(strAuxiliar) {
         ...
         if (folderId != '' && folderId != null) {
             ...
             var fileName = 'Temp' + '_' + 'CajaYBancosCtaCte' + '_' + 'MPRD'+ '_' + '.txt';
