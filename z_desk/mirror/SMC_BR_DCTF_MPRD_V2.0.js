 ...
 * @NModuleScope Public
 */
 define(['N/search', 'N/log', 'N/file', 'N/runtime', "N/record", "N/task", "/SuiteBundles/Bundle 37714/Latam_Library/LMRY_LibraryReport_LBRY_V2.js",
 "/SuiteBundles/Bundle 37714/Latam_Library/LMRY_libSendingEmailsLBRY_V2.0.js"],
 
   function(search, log, fileModulo, runtime, recordModulo, task,libreria, libFeature) {
     /**
    ...
    // var param_Excel = "F";
     ...
     function getInputData() {
       try {
         ...
         log.debug('featureTaxCalendars',featureTaxCalendars);
     ...
     function map(context) {
       try {
         ...
         if (arrTemp["isError"] == "T") {
           ...
           /* PARA E-PAYMENT */
           if (arrTemp[7] == 'DF') {
             //log.debug("arrTemp antes de DF",arrTemp);
             ...
             if (jsonDataPago != null) {
               ...
               var multaJuros = buscarBillMultaJuros(arrTemp[13], arrTemp[10]);
 
               if (multaJuros.length != 0) {
                 log.debug('si hay multas y juros', arrTemp);
                 var montosMultaJuros = obtenerMontosMultaJuros(multaJuros[0]);
                 var itemsMultaJuros = obtenerItemsMultaJuros(arrTemp[8]);
 
                 for (var i = 0; i < montosMultaJuros.length; i++) {
                   if (montosMultaJuros[i][0] == itemsMultaJuros[0]) { //juros
                     arrTemp[5] = montosMultaJuros[i][1];
                   } else if (montosMultaJuros[i][0] == itemsMultaJuros[1]) { //multa
                     arrTemp[6] = montosMultaJuros[i][1];
                   }
                 }
               } else {
                 log.debug('no hay multas y juros', arrTemp);
               }
           ...
           if (param_Excel == 'T') {
             ...
             if (arrTemp[0] == 'VendPymt') {
               if (arrTemp[7] == 'DF') {
                 /***** E-pay DARF(R11) *****/
                 key = arrTemp[13]; //id bill darf
               } else {
                 /***** Importaciones Proceso Automatico (R10) *****/
                 key = arrTemp[0] + '|' + arrTemp[2] + '|' + arrTemp[3] + '|' + arrTemp[8]; // type|tributo|receita|id subsi
               }
             }
             if (arrTemp[0] == 'Journal') { //Journals IOF CIDE para Proceso Manual (R10 R11)/para Automatico(solo R11)
           ...
           if (arrTransaction[0][0] == 'VendBill' || arrTransaction[0][0] == 'CustInvc') {
             ...
             if (arrTransaction[0][7] == 'DF') {
               ...
               for (var i = 0; i < arrTransaction.length; i++) {
                 ...
                 arrAuxiliar[13] = arrTransaction[i][5]; //Juros
                 arrAuxiliar[14] = arrTransaction[i][6]; //multas
     ...
     function summarize(context) {
       try {
         ...
         log.debug('Array Bill Payment Summarize', arrBillPayment); //Impuestos de importacion o EPayments
     ...
     function buscarBillMultaJuros(idBillDarf, idBillPay) {
       var data = new Array();
       ...
       var vendorbillSearchObj = search.create({
         ...
         filters: [
           ...
           ["voided", "is", "F"],
           "AND",
           ["formulatext: {custbody_lmry_reference_transaction.id}", "is", idBillDarf],
           "AND",
           ["mainline", "is", "T"],
           "AND",
           ["formulatext: {applyingtransaction.internalid}", "is", idBillPay]
         ],
         settings: [
           search.createSetting({
             name: 'consolidationtype',
       ...
       vendorbillSearchObj.run().each(function(result) {
         var columns = result.columns;
         data.push(result.getValue(columns[0]));
         data.push(result.getValue(columns[1]));
         return true;
       });
     ...
     function obtenerItemsMultaJuros(subsidiary_id) {
       var items = new Array();
       var customrecord_lmry_setup_tax_subsidiarySearchObj = search.create({
         type: "customrecord_lmry_setup_tax_subsidiary",
         filters: [
           ["custrecord_lmry_setuptax_subsidiary", "anyof", param_Subsi]
         ],
         columns: [
           search.createColumn({
             name: "formulatext",
             formula: "{custrecord_lmry_setuptax_br_ap_interest.id}",
             label: "0. Item ID Juros"
           }),
           search.createColumn({
             name: "formulatext",
             formula: "{custrecord_lmry_setuptax_br_ap_multa.id}",
             label: "1. Item ID Multas"
           })
         ]
       });
 
       customrecord_lmry_setup_tax_subsidiarySearchObj.run().each(function(result) {
         var columns = result.columns;
         items.push(result.getValue(columns[0]));
         items.push(result.getValue(columns[1]));
         return true;
       });
     ...
     function obtenerMontosMultaJuros(idBillMultaJuros) {
       ...
       var vendorbillSearchObj = search.create({
         ...
         filters: [
           ...
           ["voided", "is", "F"],
           "AND",
           ["internalid", "anyof", idBillMultaJuros],
           "AND",
           ["mainline", "is", "F"],
           "AND",
           ["taxline", "is", "F"]
       ...
       vendorbillSearchObj.run().each(function(result) {
         ...
         arrTemp.push(redondear(result.getValue(columns[0])));
         arrTemp.push(redondear(result.getValue(columns[1])));
         montos.push(arrTemp);
         return true;
       });
     ...
     function LlamarSchedule(id_archivo) {
       var params = {};
       if (param_Excel == 'T') {
         params['custscript_smc_br_dctf_xls_periodo'] = param_Periodo;
         params['custscript_smc_br_dctf_xls_featureid'] = param_Feature;
         if (feature_Subsi) {
           params['custscript_smc_br_dctf_xls_subsid'] = param_Subsi;
         }
         if (feature_Multi) {
           params['custscript_smc_br_dctf_xls_multibook'] = param_Multi;
         }
         //parametro de archivo temporal
         params['custscript_smc_br_dctf_xls_fileid'] = id_archivo;
         params['custscript_smc_br_dctf_xls_tipo_decla'] = param_Type_Decla;
         params['custscript_smc_br_dctf_xls_num_rec'] = param_Num_Recti;
         params['custscript_smc_br_dctf_xls_logid'] = param_RecorID;
         params['custscript_smc_br_dctf_xls_lucro'] = param_Lucro_Conta;
         params['custscript_smc_br_dctf_xls_monto_adic'] = param_Monto_Adicional;
         params['custscript_smc_br_dctf_xls_monto_excl'] = param_Monto_Excluyente;
 
         id = 'customscript_smc_br_dctf_xls_schdl';
         deploy = 'customdeploy_smc_br_dctf_xls_schdl';
       } else {
         params['custscript_smc_br_dctf_dec_periodo'] = param_Periodo;
         params['custscript_smc_br_dctf_dec_featureid'] = param_Feature;
         if (feature_Subsi) {
           params['custscript_smc_br_dctf_dec_subsi'] = param_Subsi;
         }
         if (feature_Multi) {
           params['custscript_smc_br_dctf_dec_multi'] = param_Multi;
         }
         //parametro de archivo temporal
         params['custscript_smc_br_dctf_dec_archivo'] = id_archivo;
         params['custscript_smc_br_dctf_dec_decla'] = param_Type_Decla;
         params['custscript_smc_br_dctf_dec_recti'] = param_Num_Recti;
         params['custscript_smc_br_dctf_dec_recorid'] = param_RecorID;
         params['custscript_smc_br_dctf_dec_lucro_conta'] = param_Lucro_Conta;
         params['custscript_smc_br_dctf_dec_monto_adic'] = param_Monto_Adicional;
         params['custscript_smc_br_dctf_dec_monto_excluy'] = param_Monto_Excluyente;
 
         id = 'customscript_smc_br_dctf_dec_schdl';
         deploy = 'customdeploy_smc_br_dctf_dec_schdl';
       }
     ...
     function SaveFileAux(contenido) {
       ...
       if (folderId != '' && folderId != null) {
         ...
         var fileName = 'SMC_ArchivoTemporalDCTF.txt';
         // Crea el archivo
