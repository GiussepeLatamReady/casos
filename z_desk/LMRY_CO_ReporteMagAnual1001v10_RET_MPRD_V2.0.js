...
define(['N/record', 'N/task', 'N/runtime', 'N/file', 'N/search', 'N/encode',
        'N/format', 'N/log', './CO_Library_Mensual/LMRY_CO_Reportes_LBRY_V2.0.js', "/SuiteBundles/Bundle 37714/Latam_Library/LMRY_libSendingEmailsLBRY_V2.0.js"
    ],

    function(record, task, runtime, file, search, encode, format, log, libreria, libFeature) {
        ...
        var hasAccountMappignFeature = runtime.isFeatureInEffect({
            feature: "Coaclassificationmanagement"
        });
        ...
        function obtenerTaxReclasificado(idOrigen) {
            ...
            var customrecord_lmry_co_wht_reclasification_search = search.create({
                ...
                filters: [
                    ...
                    /*,  
                                        "AND", ["formulatext: {custrecord_co_reclasification_return}","doesnotcontain","null"]*/
                ],
                columns: [
                    search.createColumn({
                        name: "formulatext",
        ...
        function getJournalsAMano() {
            ...
            pagedData.pageRanges.forEach(function(pageRange) {
                ...
                page.data.forEach(function(result) {
                    ...
                    auxArray[11] = isPrimary;
                    auxArray[12] = 'journalsaMAno';

                    //var auxColum1 = result.getValue(result.columns[10]);
                    // if (Number(auxColum1) > 0) {
                    //     auxArray[6] = auxColum1;
                    // }
        ...
        function getCreditNotes() {
            ...
            if (arrIdOrigen.length) {
                ...
                // var currencyAmount = search.createColumn({
                //     name: "formulanumeric",
                //     formula: 'NVL({custrecord_lmry_br_transaction.custrecord_lmry_amount_local_currency},0)'
                // });
                // savedSearch.columns.push(currencyAmount);
        ...
        function getCreditNoteAccountsDetail(contextValue) {
            ...
            try {
                ...
                if (contextValue[1] == 'Journal') {
                    ...
                    if (!isPrimary && hasAccountMappignFeature) {
                        //ChangeAccount(sourceid, accbook, date, subsi)
                        //["","Journal","ReteIVA","","","",5.14,"2052","","29/03/2016","1354",true,"xLineas"]
                        account = ChangeAccountJournal(account, paramMultibook, contextValue[9], paramSubsidiary) || account;
                    }
                } else {
                    ...
                    if (objResult && objResult.length) {
                        ...
                        if (TypeAppTransaction == 'Journal' || (TypeAppTransaction == 'WHT by Transaction' && (contextValue[2] == 'Auto ReteFTE' || contextValue[2] == 'Auto ReteIVA'))) {
                            ...
                            if (!isPrimary && hasAccountMappignFeature) {
                                //ChangeAccount(sourceid, accbook, date, subsi)
                                var account = [
                                    ChangeAccountJournal(creditAcc, paramMultibook, contextValue[9], paramSubsidiary) || creditAcc,
                                    ChangeAccountJournal(debitAcc, paramMultibook, contextValue[9], paramSubsidiary) || debitAcc
                                ];
                            } else {
                            ...
                            if (!isPrimary && hasAccountMappignFeature) {
                                //ChangeAccount(sourceid,accbook,date,type,subsi)
                                account = ChangeAccountItem(account, paramMultibook, contextValue[9], contextValue[0], paramSubsidiary) || account;
                            }
                        }
                    }
                }
                    ...
                    if (objResult_2 && objResult_2.length) {
                        ...
                        for (var j = 0; j < objResult_2.length; j++) {
                            ...
                            if (aux_info.length != 0) {
                                ...
                                // if (TypeAppTransaction == 'WHT by Transaction' && contextValue[2] == 'ReteFTE' && objResult_2[j].getValue(columns[2]).split(',').indexOf('5') >= 0) {
                                //     resultArray[12] = contextValue[6];
                                // } else if ((TypeAppTransaction == 'Journal' || contextValue[1] == 'Journal') && contextValue[2] == 'ReteFTE' && objResult_2[j].getValue(columns[2]).split(',').indexOf('6') >= 0) {
                                //     resultArray[13] = contextValue[6];
                                // } else if (TypeAppTransaction == 'WHT by Transaction' && contextValue[2] == 'ReteIVA' && objResult_2[j].getValue(columns[2]).split(',').indexOf('7') >= 0) {
                                //     resultArray[14] = contextValue[6];
                                // } else if ((TypeAppTransaction == 'Journal' || contextValue[1] == 'Journal') && contextValue[2] == 'ReteIVA' && objResult_2[j].getValue(columns[2]).split(',').indexOf('8') >= 0) {
                                ...
                                if (ReteFteTipo1_2 && typeOfRet.indexOf('5') >= 0) {
                                    ...
                                    resultArray[13] = contextValue[6];
                                } else if (ReteIvaTipo1_2 && typeOfRet.indexOf('7') >= 0) {
                                    resultArray[14] = contextValue[6];
                                } else if ((autoReteIvaTipo1 || autoReteIvaTipo2) && typeOfRet.indexOf('8') >= 0) {
                                    resultArray[15] = contextValue[6];
                                }
                ...
                if (objResult && objResult.length) {
                    ...
                    for (var j = 0; j < objResult.length; j++) {
                        ...
                        if (aux_arrinformation.length != 0) {
                            ...
                            //Rte IVa
                            if (accountsJson[objResult[j].getValue(columns[0])] == 'RteIVA' && contextValue[3] == 'WHT' && objResult[j].getValue(columns[2]).split(',').indexOf('7') >= 0) {
                                resultArray[14] = contextValue[1] || 0;
                            } else if (accountsJson[objResult[j].getValue(columns[0])] == 'RteIVA' && contextValue[3] == 'AutoWHT' && objResult[j].getValue(columns[2]).split(',').indexOf('8') >= 0) {
        ...
        function ChangeAccountJournal(sourceid, accbook, date, subsi) {

            //log.error('date', date);

            var globalaccountmappingSearchObj = search.create({
                type: "globalaccountmapping",
                filters: [
                    ["formulatext: {sourceaccount.id}", "is", sourceid],
                    "AND", ["accountingbook", "anyof", accbook],
                    "AND", ["effectivedate", "onorafter", date],
                    "AND", ["enddate", "onorbefore", date],
                    "AND", ["subsidiary", "anyof", subsi]
                ],
                columns: [
                    search.createColumn({
                        name: "formulatext",
                        formula: "{sourceaccount.id}",
                        label: "Formula (Text)"
                    }),
                    search.createColumn({
                        name: "formulatext",
                        formula: "{destinationaccount.id}",
                        label: "Formula (Text)"
                    }),
                    search.createColumn({ name: "destinationaccount", label: "Destination Account" })
                ]
            });

            var objResult = globalaccountmappingSearchObj.run().getRange(0, 1000);
            //log.error('objResult.length', objResult.length);
            if (objResult && objResult.length) {
                var columns = objResult[0].columns;
                var NewAccount = objResult[0].getValue(columns[1]);
            } else {
                var NewAccount = '';
            }
            return NewAccount;
        ...
        function ChangeAccountItem(sourceid, accbook, date, type, subsi) {

            //log.error('date', date);

            var itemaccountmappingSearchObj = search.create({
            ...
            var objResult = itemaccountmappingSearchObj.run().getRange(0, 1000);
            //log.error('objResult.length', objResult.length);
            if (objResult && objResult.length) {
                var columns = objResult[0].columns;
                var NewAccount = objResult[0].getValue(columns[1]);
        ...
        function getAccounts(contextValue) {
            ...
            if (Math.abs(contextValue[1]) > 0) {
                if (contextValue[3] == 'WHT') {
                    ...
                    if (accIVA != '') {
                        if (!contextValue[16] && hasAccountMappignFeature) {
                            //ChangeAccount(sourceid,accbook,date,type,subsi)
                            accIVA = ChangeAccountItem(accIVA, paramMultibook, contextValue[17], contextValue[0], paramSubsidiary) || accIVA;
                        }
                    ...
                    if (!contextValue[16] && hasAccountMappignFeature) {
                        //ChangeAccount(sourceid, accbook, date, subsi)
                        accountiva1 = accountiva1 != '' ? ChangeAccountJournal(accountiva1, paramMultibook, contextValue[17], paramSubsidiary) || accountiva1 : accountiva1;
                        accountiva2 = accountiva2 != '' ? ChangeAccountJournal(accountiva2, paramMultibook, contextValue[17], paramSubsidiary) || accountiva2 : accountiva2;
                    }
                    if (accountiva1 != '')
            ...
            if (Math.abs(contextValue[2]) > 0) {
                if (contextValue[4] == 'WHT') {
                    ...
                    if (accFTE != '') {
                        if (!contextValue[16] && hasAccountMappignFeature) {
                            //ChangeAccount(sourceid,accbook,date,type,subsi)
                            accFTE = ChangeAccountItem(accFTE, paramMultibook, contextValue[18], contextValue[0], paramSubsidiary) || accFTE;
                        }
                    ...
                    if (!contextValue[16] && hasAccountMappignFeature) {
                        //ChangeAccount(sourceid, accbook, date, subsi)
                        accountrtf1 = accountrtf1 != '' ? ChangeAccountJournal(accountrtf1, paramMultibook, contextValue[18], paramSubsidiary) || accountrtf1 : accountrtf1;
                        accountrtf2 = accountrtf2 != '' ? ChangeAccountJournal(accountrtf2, paramMultibook, contextValue[18], paramSubsidiary) || accountrtf2 : accountrtf2;
                    }
                    if (accountrtf1 != '')
